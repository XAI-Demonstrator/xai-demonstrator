<template>
  <div>
      <section v-show="!explanation" class="xd-section xd-light" style="height:55px">
         <section v-show="!user_country_answer">
            {{ msg }}
         </section>   
        <section v-show="user_country_answer && !user_city_answer">
        <p class="short-text" v-if="user_country_answer == label_country" >Your answer: {{user_country_answer}} is right</p>
      ...
      <section v-show="user_city_answer && !prediction_city">
        <p class="short-text" v-if="user_city_answer == label_city" >Your answer: {{user_city_answer}} is right</p>
        <p v-else>Your answer: {{user_city_answer}} is wrong, the right answer ist: {{label_city}}</p>
      </section>
      <section v-show="prediction_city">
        <p class="short-text" v-if="prediction_city == label_city" >The AI answer: {{prediction_city}} is right</p>
