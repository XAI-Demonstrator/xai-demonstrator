...
def read_imagefile2(file):
    ...
    img_array = cv2.cvtColor(cv2.imdecode(nparr, cv2.IMREAD_ANYCOLOR),
                             cv2.COLOR_BGR2RGB)  # read in the image, convert to grayscale
    return img_array


def prepare(filepath):
    IMG_SIZE = 224  # 50 in txt-based
    img_array = cv2.cvtColor(cv2.imread(filepath, cv2.IMREAD_ANYCOLOR),
                             cv2.COLOR_BGR2RGB)  # read in the image, convert to grayscale
    new_array = cv2.resize(img_array, (IMG_SIZE, IMG_SIZE))  # resize image to match model's expected sizing
    return new_array.reshape(-1, IMG_SIZE, IMG_SIZE, 3) / 224  # return the image with shaping that TF wants.
